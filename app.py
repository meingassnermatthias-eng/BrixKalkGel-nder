...
class BrixDatabase:
    def __init__(self, preisfaktor=1.0):
        self.faktor = preisfaktor # Globaler Preisfaktor (z.B. 1.05 für +5%)
        ...
        # Dynamische Preisanpassung anwenden
        self.modelle = {}
...
def display_pdf(file_path):
    if os.path.exists(file_path):
        ...
        st.info("Bitte lade deine Preisliste als 'preisliste.pdf' auf GitHub hoch, damit sie hier angezeigt wird!")

# --- PDF EXPORT FUNCTION (FPDF) ---
def create_sales_pdf(res, db):
    ...
    pdf.cell(0, 8, txt(f"Modell: {d['modell']} ({d['farbe']})"), ln=True)
    pdf.set_font("Arial", '', 10)
    ...
    row("Material Geländer-Felder", res['k_felder'])
...
# --- MAIN APP ---
def main():
    st.set_page_config(page_title="Brix Kalkulator V5", page_icon="🏗️", layout="wide")
    
    # Sidebar für PREIS-ANPASSUNGEN (Admin-Bereich)
    with st.sidebar:
        st.header("⚙️ Einstellungen")
        st.write("Falls Brix die Preise erhöht:")
        preisfaktor = st.number_input("Globaler Preis-Faktor:", 1.00, 2.00, 1.00, 0.01, help="1.00 = Originalpreis. 1.05 = +5% Preiserhöhung auf alles.")
        if preisfaktor > 1.0:
            st.warning(f"⚠️ Preise werden mit Faktor {preisfaktor} berechnet!")
        
        st.divider()
        st.info("Lade 'preisliste.pdf' auf GitHub hoch, um sie rechts anzuzeigen.")

    db = BrixDatabase(preisfaktor)
    st.title("🏗️ Brix Kalkulator & Katalog Viewer")

    # SPLIT SCREEN: Links Kalkulator, Rechts PDF
    ...
    with col_pdf:
        st.subheader("📋 Original Preisliste")
